...
class Term:
    ...
    def get_term_idx(self, term):
        """Returns the index of a subterm."""
    ...
    def copy(self):
        """Makes a copy of a term, without fucking up the recursion. Use this instead of copy.deepcopy"""
        new = Term()
        new.owner_term = self.owner_term
        #Recursion
        for i in range(self.num_terms()):
            new.sub_terms.append(self.sub_terms[i].copy())
        new.set_sub_term_owners()
        new.vars = copy.copy(self.vars)
        new.powers = copy.copy(self.powers)
        new.operator = self.operator
        new.num = self.num
        return new
    
    def __truediv__(self, other, fraction=False):
        """divide self by other.

        Will return both numerator and denominator if fraction is false."""
        numer = self.copy()
        denom = other.copy()
        #Reduce var powers
        for var in numer.get_same_vars(denom):
            numerpow = numer.get_power_for_var(var)
            denompow = denom.get_power_for_var(var)
            #Get smallest power
            if (numerpow < denompow):
                smallpow = numerpow
            else:
                smallpow = denompow
            #Reduce variable powers
            numerpos = numer.get_location_for_var(var)
            denompos = denom.get_location_for_var(var)
            numer.vars[numerpos] -= smallpow
            denom.vars[denompos] -= denompow
        #Reduce subterm powers
        for t in numer.get_same_subterms(denom):
            numer.get_power_for_term(t)
        #Recursion
        for t in numer.sub_terms:
            t /= denom

        #Remove subterms / vars with a power of zero
        numer.remove_zero_power_terms()
        numer.remove_zero_powers()
        if not fraction:
            return numer
        else:
            denom.remove_zero_power_terms()
            denom.remove_zero_powers()
            return numer, denom
    ...
    def div_with_subterms(self, other):
        ...
        cache_term = self.__truediv__(other, fraction=True)
        ...
        self.vars = cache_term.vars
        self.num = cache_term.num
        #self.powers = copy.deepcopy(cache_term.powers)
        self.powers = cache_term.powers
        self.operator = cache_term.operator
        #NOTE: Not sure about the following line
        #self.sub_terms = copy.deepcopy(cache_term.sub_terms)
        self.sub_terms = cache_term.sub_terms
        #print out the root term for debugging purposes
        print("cache_term operator: " + cache_term.operator + ", self operator: " + self.operator)
        print("cache_term num: " + str(cache_term.num) + ", self num: " + str(self.num))
        print("Root term: " + self.debug_get_root_term_string())
        del cache_term

        print("div_with_subterms got: " + self.get_bracket_string())

    def perform_operation(self, other):
        """Performs other's operation with self. Eg. 3 * 3 = 9."""
        if other.operator == "+":
            return self + other
        if other.operator == "-":
            return self - other
    ...
    def eq_after_invert(self, other):
        """Returns true if self and other are equal after one is divided by -1"""
    ...
    def convert_extra_floats(self, recursive=True):
        """Converts self's num from a float to an int if it doesn't have a decimal."""
        if recursive:
            for i in range(self.num_terms()):
    ...
    def update_operator(self, recursive=False):
        """Flips the operator of self if it's less than 0."""
        if recursive:
            for i in range(self.num_terms()):
    ...
    def get_same_vars(self, other):
        """Returns a dict full of arrays that contain the positions of the vars."""
        vars = {}
        for i in range(self.num_vars()):
    ...
    def get_same_vars_in_subterms(self):
        """Returns the same variables between all of self's subterms."""
        vars = {}
        for i in range(self.num_terms()):
    ...
    def dev_factor(self):
        """Like factor but with debug print statements."""
            ...
            for n in range(self.num_terms()):
                if i != n:
                    ...
                    #check term_n / -1
                        ...
                        print("divided term " + term_n.get_bracket_string() + " with subterms and now checking.")
                        if term_i.has_same_subterms(term_n):
                            #find if there's already an array in same_term_pos that contains a location of said term.
                            has_found = False
                            for term_pos_i in range(len(same_term_pos)):
                            ...
                            print("Not a match. Returning term to previous state.")
    ...
    while equation_num < strlen:
        ...
        #set .num
            ...
            if(current_term.num == 1):
                ...
                #of the number in case it's more than 1 digit - ie. 37
