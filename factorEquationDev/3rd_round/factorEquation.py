...
#            if not self.subTerms[i] == other.subTerms[i]:
        ...
        #check each term's power.
        for i in range(self.numTerms()):
            selfPwr = self.subTerms[i].power
            otherPwr = other.subTerms[i].power
            if selfPwr != otherPwr:
    ...
    def commonFactor(self, recursive = False):
        ...
        for term in range(self.numTerms()):
            if term + 1 != numTerms: #we can't get the index of term + 1 if it's outside the scope of self.subTerms.
                if self.subTerms[term] == self.subTerms[term + 1] and self.subTerms[term + 1].operator == "*":
                    print("Got two terms multiplied by each other.")
                    self.subTerms[term].power += 1
...
current_term = terms.subTerms[0]
#print("index: " + str(terms.subTerms.index(term)))
print("terms: ")
#terms.printTree()
terms_str = terms.get_bracket_string()
print(terms_str)
print("current_term:")
current_term_str = current_term.get_bracket_string()
print(current_term_str)
print("index: " + str(get_idx(terms.subTerms, current_term)))
