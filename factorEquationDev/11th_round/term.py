    ...
    while equation_num < strlen:
        ...
        #set .num
            ...
            if(terms.num == 1):
                ...
                current_term.num, num_length = get_num_at_pos(raw_equation, equation_num)
            ...
            current_term.powers.apppend(1)
            terms.owner_term.has_run_startup = True
        elif equation_type == "^":
            #tell the difference between a power acting on a variable and a power acting on an entire term
            if not is_term_power(equation_num):
                current_term.powers[-1], num_length = get_num_at_pos(raw_equation, equation_num + 1)
            else:
                current_term.sub_terms[-1].power, num_length = get_num_at_pos(raw_equation, equation_num + 1)
