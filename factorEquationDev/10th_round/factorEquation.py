...
class term:
    ...
    def invert_operator(self):
        ...
        if self.operator == "+":
            self.operator = "-"
            print("invert_operator worked.")
            return True
        elif self.operator == "-":
            self.operator = "+"
            print("invert_operator worked.")
            return True
        print("invert_operator couldn't invert the operator.")
        return False
    ...
    def __truediv__(self, other, whole_fraction=False):
        ...
        print("truediv dividing " + numerator.get_bracket_string() + " by " + denominator.get_bracket_string())
        if whole_fraction:
            print("not whole fraction.")
            lcm_term = term()
        ...
        if denominator.operator == "-":
            numerator.invert_operator()
            denominator.invert_operator()
        #NOTE: left off here. May have fixed issue with operators not being correct.
        ...
        for var in vars:
            for i in range(self.num_terms()):
                if not self.sub_terms[i].has_var(var):
                    print("Tagging " + var + " to be deleted.")
...
#    print("Checking element "+str(equation_num) + " which is of type " + equation_type)
    if equation_type == "(": #go down a level of recursion
        ...
        if terms.owner_term.has_run_startup:
            ...
            #term_pos = terms.owner_term.get_location_for_term(terms)
            terms = terms.owner_term
            terms.sub_terms.append(term())
            terms.sub_terms[-1].owner_term = terms
            terms = terms.sub_terms[-1]
