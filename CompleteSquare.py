...
maxFactorCheck = 5000
#note to self: possible way to add powers: set everything sys.argv has into one string. If a number has "^" before it, it's a power. If it doesn't, it's a base. If the number has a variable beside it, it's tied to that variable. If a variable has no number, it's number = 1. If a variable has the number 0, set it to 0.
#see which variables we have been given
if len(sys.argv) == 3:
    firstNum = float(sys.argv[1])
    secondNum = 0
    extraNum = float(sys.argv[2])
    variableName = "x" #make sure we have a variable name
elif len(sys.argv) == 4:
    firstNum = float(sys.argv[1])
    secondNum = float(sys.argv[2])
    extraNum = float(sys.argv[3])
    variableName = "x" #make sure we have a variable name
elif len(sys.argv) == 5:
    firstNum = float(sys.argv[1])
    secondNum = float(sys.argv[2])
...
#the main few lines for adding support from our FactorFloat script go here.
print("First Num: "+str(firstNum))
print("Second Num: "+str(secondNum))
print("Extra Num: "+str(extraNum))
oldA = firstNum
if firstNum != 1:
    aEqual = False
    print("a != 1. Attempting to fix.")
    secondNum = secondNum/firstNum
    firstNum = 1.0
    print("Got new First Num: "+str(firstNum))
    print("Got new Second Num: "+str(secondNum))
else:
    aEqual = True
thirdNum = (secondNum/2)*(secondNum/2)
print("Third Num: "+str(thirdNum))
...
def lCM(num1,num2,max=500): #finds the lowest common multiple, checking up to max
    num1 = float(num1)
    num2 = float(num2)
...
#check if the second bracket is equal to the first bracket with the intigers flipped.
    ...
    if not firstAndSecondEqual: #if our brackets aren't the same, we can print them seperate from each other.
        if not backwards: #store what we will print in an array finalFactors, in case we want to do something with it later.
            ...
            finalFactors.append(str(float(firstBracketMult/multiplier))+variableName+"("+str(float(firstNum/firstBracket))+"x+"+str(float(secondSetNum/firstBracket))+")+"+str(secondBracket)+"("+str(float(firstSetNum/secondBracket))+"x+"+str(float(thirdNum/secondBracket)))
...
print("Factor(s):")
for finalFactor in finalFactors: #loop through all our final factors, and choose the best one.
    print(str(oldA)+finalFactor+"+"+str(extraNum-thirdNum*oldA)) #we need to subtract here because we need to add zeros to complete the square. This is the subtraction part.
